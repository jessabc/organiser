...
export default function useGetOnDragEnd(
  boards: Board[],
  currentBoard: Board,
  dispatch: Dispatch
) {
  ...
  const onDragEnd = ({ source, destination }) => {
    ...
    if (start === end) {
      ...
      setColumns((prev) =>
        // @ts-ignore
        prev.map((column) => (column.name === newCol.name ? newCol : column))
      );

      const updatedColumns = currentBoard.columns.map((column) =>
        column.name === source.droppableId ? newCol : column
      );
      ...
      let updatedColumns = currentBoard.columns.map((column) =>
        column.name === source.droppableId ? newStartCol : column
      );

      updatedColumns = updatedColumns.map((column) =>
        column.name === destination.droppableId ? newEndCol : column
      );
