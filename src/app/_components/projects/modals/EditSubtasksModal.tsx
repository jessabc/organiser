...
import { Board, IColumn, ITask } from "@/app/types/interfaces";
...
export default function EditSubtasksModal({
  task,
  numCompletedSubtasks,
  setNumCompletedSubtasks,
}: Props) {
      ...
      currentBoard.columns.forEach((column) => {
        if (column.name === originalTask.status) {
          const updatedTasks = column.tasks.map((task) =>
            task.title === originalTask.title ? updatedTask : task
          );
    ...
    if (updatedTask.status != originalTask.status) {
      currentBoard.columns.forEach((column) => {
        ...
        if (column.name === originalTask.status) {
          ...
          const updatedTasksInOriginalColumn = column.tasks.filter(
            (task) => task.title != originalTask.title
          );
          const updatedOriginalColumn = {
            ...column,
            tasks: updatedTasksInOriginalColumn,
          };
  ...
  function handleChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    setNumCompletedSubtasks((prev) => (e.target.checked ? prev + 1 : prev - 1));
  }
    ...
    return (
      ...
      >
        <div className="mb-2 text-left">
          ...
          {/* @ts-ignore   */}
          <div
            className="flex w-full h-1.5 bg-gray-400 rounded-full overflow-hidden dark:bg-gray-700 "
            role="progressbar"
          ...
          >
            <div
              className="flex flex-col justify-center rounded-full overflow-hidden bg-indigo-500   text-xs text-white text-center whitespace-nowrap transition duration-500 dark:bg-blue-500"
