...
export default function Task({ task, index }: Props) {
  ...
  const [numCompletedSubtasks, setNumCompletedSubtasks] = useState(
    getNumCompletedSubtasks(task)
  );
  ...
  const deleteProps = {
    ...
    onDelete: () =>
      deleteTask(currentBoard, task, dispatch, boards, setAllBoards),
  };
          ...
          >
            ...
            <div className="cursor-grab dahover:dark:bg-zinc-700 flex flex-col my-3  p-3 h-40 w-52 hover:shadow-lg justify-between bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-700 dark:shadow-slate-700/[.7]">
              ...
              <EditSubtasksModal
                ...
                setNumCompletedSubtasks={setNumCompletedSubtasks}
              />
