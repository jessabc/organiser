...
export default function FilterDropdown({ setFilterQuery }: Props) {
  ...
  const categoriesEl = categories.map((category) => (
    <MenuItem key={category} item={category} setFilterQuery={setFilterQuery} />
  ));

  // income and expense
  const transactionTypeEl = transactionTypesData.map((transactionType) => (
    <MenuItem
      key={transactionType}
      ...
      <Menu as="div" className="relative inline-block text-left">
        ...
        >
          <Menu.Items className="absolute left-0 sm:right-0 sm:left-auto mt-2 w-40 origin-top-right divide-y divide-gray-200 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none dark:bg-gray-800 dark:border dark:border-gray-700">
            <div className="px-1 py-1">{transactionTypeEl}</div>
            <div className="px-1 py-1">{categoriesEl}</div>
