...
export default function BoardsAccordian() {
  ...
  const boardNameEl = boards?.map(board => <BoardNameLink key={board.name} board={board}/>)
    ...
    <li className="hs-accordion" id="projects-accordion">
      ...
      <div id="projects-accordion-child" className="hs-accordion-content w-full overflow-hidden transition-[height] duration-300 hidden">
        <ul className="pt-2 ps-2">
          {boardNameEl}
          <Link href={"/projects/new"}  className="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700" >
            + New Board
          </Link > 
        </ul>
      </div>
